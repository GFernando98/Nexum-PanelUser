...
export interface EditProfessionRequest {
    id: string;
    name: string;
}
...
export class ProfessionService {
    ...
    private readonly baseUrl = `${environment.apiUrl}/api/Profession`;
    ...
    getAll(query: PaginatedQuery = {}) {
        ...
        return this.http.get<PaginatedResult<ProfessionDto>>(`${this.baseUrl}/GetAll`, { params });
    }

    private normalizeProfessionResponse(response: unknown): ProfessionDto {
    ...
    getById(id: string) {
        return this.http.get(`${this.baseUrl}/GetById/${id}`).pipe(
            map(response => this.normalizeProfessionResponse(response))
        );
    }

    create(request: CreateProfessionRequest) {
        return this.http.post(`${this.baseUrl}/Create`, request, { responseType: 'text' });
    }

    edit(id: string, request: EditProfessionRequest) {
